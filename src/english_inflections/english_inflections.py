...
class Inflector:
  ...
  The constructor reads a TSV database file "english_inflections.tsv".
  The InflectNoun, InflectVerb, InflectAdjective, InflectAdverb, and Inflect methods searches
  the database and return a map of inflected forms.  It includes the following attributes.
    - o: original form
    - np: noun prural
    - vs: verb singular 
    - vc: verb present participle (aka. continuous/gerund)
    - vp: verb past
    - vx: verb past participle
    - ajc: adjective comparative
    - ajs: adjective superative
    - avc: adverb comparative
    - avs: adverb superative
    - _ph: debug info: attribute names obtained from a phrase entry in the database
    - _pi: debug info: IDF of the phrase (aka. -log(probability))
    - _th: debug info: attribute names obtained from a token entry in the database

  Each inflection method has "fbgen" parameter.  If it is true and there's no adequate information
  for the given phrase, inflections are generated by heuristics.  You can also call
  GenerateNounPrural and etc directly for that purpose.

  The Search method searches for the original forms of the given phrase.  It returns a list of
  the base forms and thier scores (smaller is better).  To search the database for the orignal
  words, LookupPhraseInfo is useful.
  """
